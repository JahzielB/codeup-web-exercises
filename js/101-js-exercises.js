...
function timesTwoPlusThree(num1) {
    return num1 * 2 + 3;
}
...
function isVowel(vowel) {
    var answer;
    if (answer = vowel == 'a' || vowel == 'A' || vowel == 'e' || vowel == 'E' || vowel == 'i' || vowel == 'I' || vowel == 'o' || vowel == 'O' || vowel == 'u' || vowel == 'U') {
        return true;
    } else {
        return false;
    }
...
function countVowels(count) {
    var vowels = count.match(/[aeiou]/gi);
...
function startsAndEndsWithVowel(stend) {
    var vowels = stend.match(/^[aeiou].*[aeiou]$/i)
    if (vowels) {
        return true;
    } else {
        return false;
...
function longestString(x) {
    const longer = (left, right) => left.length >= right.length ? left : right;
    return x.reduce(longer);
}
...
function getUniqueValues(x) {
    var uniqueArr = x.filter((item, index) => {
...
function highestPriceBook(x) {
    var bigBook = x.reduce(function(x, y) {
...
function lowestPriceBook(x) {
    var smallBook = x.reduce(function(x, y) {
        return (x.price < y.price) ? x : y;
    });
    return smallBook;
}
...
function getAverageItemPrice(x) {
    var itemPrice = 0;
    for (var i = 0; i < x.items.length; i++) {
        itemPrice += x.items[i].price;
    };
    return itemPrice  / x.items.length;
}
...
function getAverageSpentPerItem(x) {
    ...
    var bigPrice = 0;
    for (var i = 0; i < x.items.length; i++) {
        bigPrice += x.items[i].quantity * x.items[i].price;
        totalItems += x.items[i].quantity;
    };
    return bigPrice / totalItems;
}
...
function mostSpentOnItem(x) {
    var initialValue = {"price": 0, "quantity": 0}
    var bigMoneyItem = x.reduce(function(x, y,) {
        return ((x.items.quantity * x.items.price) > (y.items.quantity * y.items.price)) ? x : y;
    }, initialValue);
    return bigMoneyItem;
}
